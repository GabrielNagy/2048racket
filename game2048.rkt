...
; (position-of-zero list n) returns the position of the n-th zero in the list
(define (position-of-zero l n)
  (cond [(null? l) #f]
        [(zero? (car l))
         (if (zero? n) 0 (+ 1 (position-of-zero (cdr l) (- n 1))))]
        [else (+ 1 (position-of-zero (cdr l) n))]))
